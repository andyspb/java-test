...
public class StartSportCamp {
  public static void main(String... args) {
    ...
    Collection<SportsCamp> sport =
        Arrays.asList(new SportsCamp("Ivan", 5), new SportsCamp(null, 15),
            new SportsCamp("Petr", 7), new SportsCamp("Ira", 10), new SportsCamp("Ivan", 17));
    String max_name = sport.stream().filter((p) -> p.getName() != null)
        .max((p1, p2) -> p1.getDay().compareTo(p2.getDay())).get().getName();
    System.out.println("max Name=" + max_name);

    String min_name = sport.stream().filter((p) -> p.getName() != null)
        .min((p1, p2) -> p1.getDay().compareTo(p2.getDay())).get().getName();
    ...
    long countNameParallel = sport.parallelStream()
        .filter((p) -> p.getName() != null && p.getName().equals("Ivan")).count();
    System.out.println("countNameParallel=" + countNameParallel);

    List<SportsCamp> onlyI =
        sport.stream().filter(p -> p.getName() != null && p.getName().startsWith("I"))
            .collect(Collectors.toList());
    System.out.println("SIZE=" + onlyI.size());

    String campPeople = sport.stream().filter(p -> p.getName() != null).map(SportsCamp::getName)
